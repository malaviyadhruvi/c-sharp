    ...
    public partial class purchase_report : Form
    {
        ...
        private void radioButton1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (radioButton1.Checked == true)
            {
                ...
                id_cmb.Items.Clear();
                id_cmb.Visible = true;
                cmd = new OleDbCommand("select pid from purchase", con);
        ...
        private void radioButton2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (radioButton2.Checked == true)
            {
                ...
                nm_cmb.Items.Clear();
                nm_cmb.Visible = true;
                cmd = new OleDbCommand("select pnm from purchase", con);
                adp = new OleDbDataAdapter(cmd);
                ds = new DataSet();
                adp.Fill(ds);
                con.Close();
        ...
        {
            string st = Application.StartupPath + "\\report\\purchase_report.rpt";
            axCrystalReport1.ReportFileName = st;

            if (radioButton1.Checked == true)
            {
                string str = Application.StartupPath + "\\report\\purchase_report.rpt";
                axCrystalReport1.ReportFileName = str;

                axCrystalReport1.SelectionFormula = "{ purchase.pid}=" + id_cmb.Text + "";
            }
            else if (radioButton2.Checked == true)
            {
                string str = Application.StartupPath + "\\report\\purchase_report.rpt";
                axCrystalReport1.ReportFileName = str;
                axCrystalReport1.SelectionFormula = "{ purchase.pnm}='" + nm_cmb.Text + "' ";
            }
            else
            {
                axCrystalReport1.SelectionFormula = "{purchase.pid}>0";
            }
