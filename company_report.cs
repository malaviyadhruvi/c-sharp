    ...
    public partial class company_report : Form
    {
        ...
        private void radioButton1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (radioButton1.Checked == true)
            {
                ...
                id_cmb.Items.Clear();
                id_cmb.Visible = true;
                cmd = new OleDbCommand("select cid from company", con);
        ...
        private void radioButton2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (radioButton2.Checked == true)
            {
                ...
                nm_cmb.Items.Clear();
                nm_cmb.Visible = true;
                cmd = new OleDbCommand("select cnm from company", con);
                adp = new OleDbDataAdapter(cmd);
                ds = new DataSet();
                adp.Fill(ds);
                con.Close();
        ...
        {
            string st = Application.StartupPath + "\\report\\company_report.rpt";
            axCrystalReport1.ReportFileName = st;

            if (radioButton1.Checked == true)
            {
                string str = Application.StartupPath + "\\report\\company_report.rpt";
                axCrystalReport1.ReportFileName = str;

                axCrystalReport1.SelectionFormula = "{ company.cid}=" + id_cmb.Text + "";
            }
            else if (radioButton2.Checked == true)
            {
                string str = Application.StartupPath + "\\report\\company_report.rpt";
                axCrystalReport1.ReportFileName = str;
                axCrystalReport1.SelectionFormula = "{ company.cnm}='" + nm_cmb.Text + "' ";
            }
            else
            {
                axCrystalReport1.SelectionFormula = "{ company.cid}>0";
            }
